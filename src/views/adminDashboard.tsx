...
import { UsersList } from '../service/pizzaService';
...
export default function AdminDashboard(props: Props) {
  ...
  React.useEffect(() => {
    (async () => {
      setUsersList(await pizzaService.listUsers(userPage, 10, '*'));
    })();
  }, [props.user, userPage]);
          ...
          <div className="bg-neutral-100 overflow-clip my-4">
            <div className="flex flex-col">
              <div className="-m-1.5 overflow-x-auto">
                ...
                <h3 className="text-neutral-100 text-xl mt-6">Users</h3>
                <div className="bg-neutral-100 overflow-clip my-4">
                  <div className="flex flex-col">
                    <div className="-m-1.5 overflow-x-auto">
                      <div className="p-1.5 min-w-full inline-block align-middle">
                            ...
                            {usersList.users.map((u, uindex) => (
                              <tbody key={uindex} className="divide-y divide-gray-200">
                                <tr className="border-neutral-500 border-t-2">
                                  ...
                                  <td className="text-start px-2 whitespace-nowrap text-sm font-normal text-gray-800">{u.roles?.map(r=>r.role).join(', ')}</td>
                                  <td className="px-6 py-1 whitespace-nowrap text-end text-sm font-medium">
                                    <button type="button" className="px-2 py-1 inline-flex items-center gap-x-2 text-sm font-semibold rounded-lg border border-1 border-red-400 text-red-400  hover:border-red-800 hover:text-red-800" onClick={() => deleteUser(u)}>
                                      <span>Delete</span>
                                    </button>
                                  </td>
                                </tr>
                            ...
                            <tfoot>
                              <tr>
                                ...
                                <td colSpan={2} className="text-end text-sm font-medium">
                                  ...
                                  <button className="w-12 p-1 text-sm font-semibold rounded-lg border border-transparent bg-white text-grey border-grey m-1 hover:bg-orange-200 disabled:bg-neutral-300" onClick={() => setUserPage(userPage + 1)} disabled={!usersList.more}>
                                    »
                                  </button>
                                </td>
                              </tr>
